                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @forelse ($users as $user)
                        <tr class="hover:bg-gray-50" wire:key="{{ $user->id }}">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ !empty($user->email_verified_at) ? 'bg-purple-100 text-purple-800' : 'bg-red-100 text-red-800' }}">
                                    {{ $user->showVerificationStatus() }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('user.edit', $user->id) }}" class="text-blue-600 hover:text-blue-900" title="Edit" wire:navigate>
                                        <x-heroicon-o-pencil-square class="h-5 w-5" />
                                    </a>
                                    <button type="button" wire:click.prevent="deleteConfirmation({{ $user->id }})" class="text-red-600 hover:text-red-900" title="Delete">
